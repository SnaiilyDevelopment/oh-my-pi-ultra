...
export interface ToolResultProjection { content: Array<TextContent | ImageContent>; meta: ToolResultIntelligenceMeta; projected: boolean; }
...
interface ResultLike { content: Array<TextContent | ImageContent>; details?: unknown; isError?: boolean; }
const cache = new WeakMap<object, { input: string; projection: ToolResultProjection }>();
const MEDIUM = 12 * 1024;
const LARGE = 32 * 1024;
const EXCERPTS = 8;
const textOf = (r: ResultLike) => r.content.filter((x): x is TextContent => x.type === "text").map(x => x.text).join("\n");
...
function projectShell(text: string, state: ToolResultState, r: ResultLike, params: unknown): string {
	...
	return [
		`COMMAND ${state}`,
		command(params) ? `command: ${command(params)}` : undefined,
		typeof details(r).exitCode === "number" ? `exit: ${details(r).exitCode}` : undefined,
		typeof details(r).wallTimeMs === "number" ? `duration: ${Number(details(r).wallTimeMs).toFixed(1)}ms` : undefined,
		errorLines.length ? `errors:\n${errorLines.join("\n")}` : undefined,
		warnings.length ? `warnings:\n${warnings.join("\n")}` : undefined,
		paths([...errorLines, ...warnings].join("\n")).length ? `paths: ${paths([...errorLines, ...warnings].join("\n")).join(", ")}` : undefined,
...
function projectSearch(text: string, r: ResultLike): string {
	const d = details(r);
	const matchCount = typeof d.matchCount === "number" ? d.matchCount : undefined;
	const fileCount = typeof d.fileCount === "number" ? d.fileCount : undefined;
...
function importance(state: ToolResultState, kind: ToolProjectionKind, text: string, r: ResultLike): ToolResultImportance {
	...
	if (kind === "search" && (details(r).matchCount as number | undefined ?? 0) > 0) return "HIGH";
	if (kind === "git" && /\b(?:conflict|unmerged)\b/i.test(text)) return "CRITICAL";
	return bytes(text) >= envInt("PI_TOOL_INTELLIGENCE_HUGE_BYTES", 128 * 1024) ? "LOW" : bytes(text) >= MEDIUM ? "MEDIUM" : "LOW";
}
...
export function projectToolResult(tool: object, toolName: string, params: unknown, result: AgentToolResult): ToolResultProjection {
	...
	const r = result as AgentToolResult & ResultLike;
	const text = textOf(r);
	const kind = kindFor(toolName, params);
	const state = stateFor(r, kind);
	...
	const meta: ToolResultIntelligenceMeta = {
		importance: importance(state, kind, text, r),
		state,
		projection: kind,
		rawBytes: bytes(text),
		projectedBytes: bytes(projectedText),
		compressionRatio: bytes(text) > 0 ? bytes(projectedText) / bytes(text) : 1,
		projectionLatencyMs: performance.now() - started,
		duplicate: false,
		fullOutputAvailable: Boolean(digest),
		fullOutputArtifactId: digest,
	};
...
export function createToolIntelligenceTelemetry(): ToolIntelligenceTelemetry {
...
export function recordToolIntelligence(telemetry: ToolIntelligenceTelemetry, toolName: string, meta: ToolResultIntelligenceMeta, duplicate = false): void {
	...
	item.calls++;
	item.rawOutputBytes += meta.rawBytes;
	item.modelFacingBytes += duplicate ? 0 : meta.projectedBytes;
	item.projectedCalls += meta.compressionRatio < 0.999 || duplicate ? 1 : 0;
	item.duplicatesSuppressed += duplicate ? 1 : 0;
	telemetry.byTool[toolName] = item;
	telemetry.calls++;
	telemetry.rawOutputBytes += meta.rawBytes;
	telemetry.modelFacingBytes += duplicate ? 0 : meta.projectedBytes;
	telemetry.projectedCalls += meta.compressionRatio < 0.999 || duplicate ? 1 : 0;
	telemetry.duplicatesSuppressed += duplicate ? 1 : 0;
	telemetry.projectionLatencyMs += meta.projectionLatencyMs;
	if (meta.fullOutputAvailable) telemetry.fullOutputRetrievals++;
}
