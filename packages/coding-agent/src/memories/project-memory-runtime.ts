...
function workflowCandidate(agent: Agent, task: string, fingerprint: string): MemoryCandidate | undefined {
	...
	const checks = verification.plan?.checks?.map(check => check.name).filter((name): name is string => Boolean(name));
...
async function capture(agent: Agent, task: string, store: ProjectMemoryStore, fingerprint: string, runtime: ProjectMemoryRuntime): Promise<void> {
	const candidates = [userInstructionCandidate(task, fingerprint), toolingCandidate(agent, fingerprint), workflowCandidate(agent, task, fingerprint), failureCandidate(agent, task, fingerprint)].filter((item): item is MemoryCandidate => Boolean(item));
	for (const candidate of candidates) {
		...
		if (!result.accepted) {
			...
			runtime.telemetry.rejectionReasons[reason] = (runtime.telemetry.rejectionReasons[reason] ?? 0) + 1;
			continue;
		}
...
function patch(): void {
	...
	(target as any).prompt = async function projectMemoryPrompt(this: Agent, ...args: unknown[]) {
		...
		try {
			const loaded = await loadMemoryContext(this, task, store, fingerprint);
			runtime.telemetry.retrieved = loaded.retrieved;
			runtime.telemetry.notRetrieved = loaded.notRetrieved;
			runtime.telemetry.memoryContextTokens = loaded.tokens;
			runtime.telemetry.lookupLatencyMs = loaded.latencyMs;
			if (loaded.message) {
