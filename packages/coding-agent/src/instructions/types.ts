...
export interface InstructionTaskProfile {
	complexity: "SIMPLE" | "NORMAL" | "COMPLEX" | "VERY_COMPLEX";
	workflow?: {
		plan?: boolean;
		architecture?: boolean;
		verification?: "basic" | "standard" | "deep" | "final";
		reasoningDepth?: string;
	};
	kind?: InstructionTaskKind;
}

export interface InstructionFailureState {
	present: boolean;
	category?: string;
	check?: string;
	summary?: string;
...
export interface InstructionVerificationState {
	state?: string;
...
export interface InstructionModelState {
...
export interface InstructionState {
	task?: InstructionTaskProfile;
	phase?: InstructionPhase;
	lastAction?: string;
	objective?: string;
	failure?: InstructionFailureState;
	verification?: InstructionVerificationState;
	contextPressure?: number;
	model?: InstructionModelState;
	toolNames?: readonly string[];
	untrustedContentPresent?: boolean;
	specialistRole?: SpecialistInstructionRole;
}
...
export interface InstructionLayer {
	...
	readonly priority: InstructionPriority;
	readonly text: string;
}

export interface InstructionTelemetry {
	readonly baseTokens: number;
	readonly dynamicTokens: number;
	readonly phaseTokens: number;
	readonly specialistTokens: number;
	readonly totalInstructionTokens: number;
	readonly duplicateInstructionsRemoved: number;
	readonly omittedOptionalInstructions: number;
	readonly compositionLatencyMs: number;
}

export interface ComposedInstructions {
	readonly text: string;
	readonly layers: readonly InstructionLayer[];
	readonly telemetry: InstructionTelemetry;
}

export interface InstructionBudget {
	/** Approximate maximum instruction tokens. */
