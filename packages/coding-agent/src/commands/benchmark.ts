...
export default class Benchmark extends Command {
	...
	static flags = {
		...
		"success-threshold": Flags.float({ description: "Allowed Ultra success-rate decrease before flagging regression" }),
		"token-threshold": Flags.float({ description: "Allowed Ultra/base mean-token ratio" }),
		"latency-threshold": Flags.float({ description: "Allowed Ultra/base mean-latency ratio" }),
		"model-call-threshold": Flags.float({ description: "Allowed Ultra/base mean-model-call ratio" }),
		"tool-call-threshold": Flags.float({ description: "Allowed Ultra/base mean-tool-call ratio" }),
	};
	static examples = [
		"omp benchmark list",
		"omp benchmark task debugging-root-cause",
		"omp benchmark run --repository . --baseline-command 'omp launch --print' --ultra-command 'omp-ultra launch --print' --runs 3",
