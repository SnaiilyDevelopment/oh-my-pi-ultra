...
function rank(decision: ResearchDecision): number { return decision === "NO_RESEARCH" ? 0 : decision === "TARGETED_RESEARCH" ? 1 : decision === "DEEP_RESEARCH" ? 2 : 3; }
...
export function initialResearchState(task: string, classification?: TaskClassification, local?: ResearchLocalEvidence, orchestration?: OrchestrationState): ResearchState {
	...
	return { classification, objective: decision === "NO_RESEARCH" || decision === "BLOCKED" ? undefined : buildResearchObjective(task, decision, local), decision, attempts: 0, queryFingerprints: [], cacheHits: 0 };
}
...
export function shouldEscalateResearch(state: ResearchState, insufficientEvidence: boolean, conflictingSources: boolean): boolean {
	...
	if (state.decision === "TARGETED_RESEARCH") {
		state.decision = "DEEP_RESEARCH";
		state.objective = buildResearchObjective(state.objective?.question ?? "", "DEEP_RESEARCH");
		return true;
	}
	return true;
}
