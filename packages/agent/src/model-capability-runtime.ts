...
function selectReasoningLevel(strategy: StrategyProfile): AgentState["thinkingLevel"] { return strategy.reasoningMode === "off" || strategy.reasoningMode === "default" ? undefined : strategy.reasoningMode; }
function publish(agent: Agent, telemetry: ModelCapabilityTelemetry): void {
	(agent.state as CapabilityState).modelCapabilities = { ...telemetry, profile: { ...telemetry.profile, reasoningLevels: [...telemetry.profile.reasoningLevels] }, strategy: { ...telemetry.strategy, reasons: [...telemetry.strategy.reasons] }, evidence: { ...telemetry.evidence } };
}
function patch(): void {
