...
import {
	createToolIntelligenceTelemetry,
	projectToolResult,
	recordToolIntelligence,
...
async function attach(agent: Agent): Promise<void> {
	...
	const remove = agent.addBeforeModelCall(async context => {
		...
		for (const message of context.messages) {
			...
			const hash = `${name}|${JSON.stringify((message as unknown as { content?: unknown }).content ?? null)}`;
			const duplicate = seen.has(hash) && projection.meta.state === "SUCCESS" && !["read", "browser", "computer", "lsp"].includes(name);
			if (duplicate) {
				projection.meta.duplicate = true;
				projection.content = [{ type: "text", text: "[unchanged since previous result]" }];
			}
			recordToolIntelligence(telemetry, name, projection.meta, duplicate);
			messages.push({ ...(message as object), content: projection.content } as AgentMessage);
			seen.add(hash);
		}
