...
const verificationHooks = new WeakMap<Agent, Set<(signal?: AbortSignal) => Promise<void> | void>>();
const originalSetOnBeforeYield = Agent.prototype.setOnBeforeYield;
...
interface VerificationRuntimeTelemetry extends VerificationTelemetry {
...
function attachContextIntelligence(agent: Agent, route: RuntimeRoute, task: string): void {
	...
	route.removeContextHook = agent.addBeforeModelCall(async context => {
		...
		context.messages = assembleContext(task, context.messages, agent.tokenizer, {
		...
		const state = agent.state as RoutedState;
		state.contextIntelligence = assembleContext(task, context.messages, agent.tokenizer, {
			complexity: route.tracker.current.complexity,
			budgetTokens: explicitBudget ?? contextBudgetForComplexity(route.tracker.current.complexity, contextWindowTokens),
			recentMessageCount: parsePositiveInteger(Bun.env.PI_CONTEXT_RECENT_MESSAGES),
			contextWindowTokens,
...
function installBeforeYieldHook(agent: Agent, hook: (signal?: AbortSignal) => Promise<void> | void): () => void {
	let hooks = verificationHooks.get(agent);
	if (!hooks) { hooks = new Set(); verificationHooks.set(agent, hooks); }
	hooks.add(hook);
	const composite = async (): Promise<void> => {
		await (async () => { for (const callback of hooks!) await callback(); })();
	};
	originalSetOnBeforeYield.call(agent, async () => {
		await composite();
	});
	return () => {
		hooks?.delete(hook);
		if (!hooks || hooks.size === 0) {
			verificationHooks.delete(agent);
			originalSetOnBeforeYield.call(agent, undefined);
		}
	};
}
async function attachVerification(agent: Agent, route: RuntimeRoute, task: string): Promise<void> {
	...
	route.removeVerificationHook = installBeforeYieldHook(agent, async () => {
		...
		const workspaceChanged = current !== runtime.baselineWorkspace;
		if (!workspaceChanged || changedFiles.length === 0) {
			runtime.lastState = "UNVERIFIED";
			runtime.telemetry = { ...runtime.telemetry, finalState: "UNVERIFIED", workspaceChanged };
			publish(agent, route);
			return;
		}
		...
		agent.followUp({ role: "user", content: buildRepairMessage(task, result.failure, runtime.previousAttempts, result.plan.checks.find(item => item.status === "skipped")?.check, route.tracker.current.complexity), timestamp: Date.now() } as AgentMessage);
		publish(agent, route);
	});
	route.removeVerificationHook = route.removeVerificationHook ?? (() => {});
}
...
async function attach(agent: Agent, task: string): Promise<RuntimeRoute> {
	...
	byAgent.get(agent)?.removeVerificationHook();
