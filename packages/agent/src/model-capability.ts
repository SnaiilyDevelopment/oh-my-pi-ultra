...
export function deriveModelCapabilities(model: Model): ModelCapabilities {
	...
	const promptCacheMode = compatValue(model, "promptCacheMode");
	const explicitBreakpoints = compatValue(model, "supportsPromptCacheBreakpoints");
	const profile: ModelCapabilities = {
		...
		streaming: "supported",
		promptCaching: explicitBreakpoints !== undefined
			? triState(explicitBreakpoints)
			: promptCacheMode === "automatic" || promptCacheMode === "explicit"
				? "supported"
...
export function createModelCapabilityTelemetry(model: Model, task: TaskClassification): ModelCapabilityTelemetry {
	const key = cacheKey(model);
	const cacheHit = capabilityCache.has(key);
...
export function recordCapabilityEvidence(telemetry: ModelCapabilityTelemetry, event: keyof CapabilityRuntimeEvidence, model?: Model): void {
	...
	if (telemetry.evidence[event] >= 3 && model) invalidateModelCapabilities(model);
}
