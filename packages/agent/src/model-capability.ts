...
function cacheKey(model: Model): string {
	return `${model.provider}\0${model.id}\0${model.api}\0${model.baseUrl}`;
}

function triState(value: unknown): CapabilityState {
	if (value === true) return "supported";
	if (value === false) return "unsupported";
	return "unknown";
}

function compatValue(model: Model, key: string): unknown {
	if (!model.compat || typeof model.compat !== "object") return undefined;
	return (model.compat as unknown as Record<string, unknown>)[key];
}
...
export function deriveModelCapabilities(model: Model): ModelCapabilities {
	...
	const reasoning = reasoningProfile(model);
	const supportsTools = triState(model.supportsTools);
	const profile: ModelCapabilities = {
		...
		streaming: "supported",
		promptCaching: triState(
			compatValue(model, "supportsPromptCacheBreakpoints") ??
			(model.compat && typeof model.compat === "object" && "promptCacheMode" in (model.compat as object) ? true : undefined),
		),
		...
		supportsSystemMessages: "supported",
	};
	capabilityCache.set(key, profile);
	return profile;
}

export function invalidateModelCapabilities(model?: Model): void {
	if (!model) capabilityCache.clear();
	else capabilityCache.delete(cacheKey(model));
}
...
export function createStrategyProfile(task: TaskClassification, capabilities: ModelCapabilities): StrategyProfile {
	...
	const contextBudget = capabilities.contextWindow && capabilities.contextWindow > 0
		? Math.max(2048, Math.min(Math.floor(capabilities.contextWindow * ratio[task.complexity]), Math.max(4096, capabilities.contextWindow - (capabilities.maxOutputTokens ?? 0))))
		: undefined;
	if (capabilities.contextWindow) reasons.push(`context window=${capabilities.contextWindow}`);

	const highest = capabilities.reasoningLevels.at(-1);
	let reasoningMode: StrategyProfile["reasoningMode"] = "default";
	if (capabilities.reasoning === "supported" && task.complexity === "SIMPLE") reasoningMode = "off";
	...
	const preferredToolChoice = capabilities.supportsToolChoice === "supported"
		&& capabilities.supportsForcedToolChoice === "supported"
		? "auto"
		: undefined;
...
export function createModelCapabilityTelemetry(model: Model, task: TaskClassification, cacheHit = false): ModelCapabilityTelemetry {
	const profile = deriveModelCapabilities(model);
	return {
		provider: model.provider,
		model: model.id,
		profile,
		strategy: createStrategyProfile(task, profile),
		evidence: { toolCallFailures: 0, malformedToolArgs: 0, providerErrors: 0, reasoningConfigFailures: 0 },
		cacheHit,
		unsupportedRequests: 0,
	};
}

export function recordCapabilityEvidence(telemetry: ModelCapabilityTelemetry, event: keyof CapabilityRuntimeEvidence): void {
	telemetry.evidence[event] += 1;
	// Require repeated evidence before invalidating a static cache entry. One transient
	// provider failure must not turn into a permanent false capability downgrade.
	if (telemetry.evidence[event] >= 3) invalidateModelCapabilities({ provider: telemetry.provider, id: telemetry.model } as Model);
}
